...
class Context:
    ...
    def create_reduct(self, options):
        ...
        self.fragments["generic_reduct"] = walker.walk(self.fragments["ehex_program"])
    ...
    def create_guessing(self, options, reduct_modals):
        ...
        if options.optimize_guessing:
            self.fragments["guessing_hints"] = fragments.guessing_hints(
                reduct_modals, semantics=options.semantics,
            )

    def compute_envelope(self, options):
        oa_walker = OverApproximationWalker().walk
        self.set_src(
            options.envelope_out.name, render(oa_walker(self.fragments["ehex_program"]))
        )
        if options.debug:
            ...
            results = clingo(str(options.envelope_out), debug=True, print_errors=True)
        else:
            results = clingo(src=self.src[options.envelope_out.name])
        envelope = set(GroundingResultWalker().walk(next(answerset.parse(results))))
        ground_modals = {literal for literal in envelope if isinstance(literal, Modal)}
        envelope -= ground_modals

        if options.debug:
            print("Envelope:", answerset.render(envelope), file=sys.stderr)
            weak_modals = (
                literal if literal.op == "M" else DefaultNegation(literal=literal)
                for literal in ground_modals
            )
            print("Ground weak modals:", answerset.render(weak_modals), file=sys.stderr)
    ...
    def render_fragments(self, options, update_src=True):
        ...
        for key in [*self.fragments]:
            ...
            self.set_src(
                "{}.fragment".format(key), render(fragment, sort=key in render_sorted)
            )
        if not update_src:
            return
        self.set_src(
        ...
        if options.optimize_guessing:
            self.append_src(
                options.guessing_out.name, self.src["guessing_hints.fragment"]
            )
    ...
    def render_subset_constraints(self, world_views, level):
        ...
        self.append_src(
            "world_views.fragment",
            render(rules(), sort=True),
...
class Solver:
    ...
    def solve(self, options):
        ...
        for level in range(context.min_level, context.max_level + 1):
            ...
            context.set_src(level_out.name, render(fragments.level_constraint(level)))
            ...
            if options.enable_queries:
                cautious = context.compute_consequences(
                    "cautious", options, extra_src=src[level_out.name], key=level
                )
                if cautious is None:
                    continue
                ...
                checking_level = len(cautious) + len(context.brave_modals) - len(brave)
            ...
            first_wv = None
            all_modals = {
                lit: modal if modal.op == "M" else DefaultNegation(literal=modal)
            ...
            other_wvs = (wv for wv in self.world_views[level] if wv is not first_wv)
            for wv in other_wvs:
                in_guess = {nkey(lit): lit for lit in guess}
                in_guess.update(in_facts)
                out_guess = {
            ...
            if options.debug:
                if self.world_views[level]:
                    ...
                    print("No world view at level {}.".format(level), file=sys.stderr)
            if level == 0 and self.world_views[level]:
                if context.max_level > 0 and options.debug:
                    print(
                        "Solved at level 0, skipping {}.".format(
                            "level 1"
                            if context.max_level == 1
                            else "levels 1-{}".format(context.max_level)
                        )
                    )
                break
